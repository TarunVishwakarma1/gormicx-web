# Magazine Hero Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the existing SaaS-style homepage with an editorial split-column hero viewport, animated with custom cursor, word reveal, stat counter, film grain, and ambient glow pulse.

**Architecture:** All work is in the `.worktrees/feat-doc-site` worktree on branch `feat/doc-site`. Four changes: (1) globals.css gets new keyframes + cursor reset, (2) two new components (`CursorGlow`, `FilmGrain`), (3) one new hook (`useCountUp`), (4) `pages/index.tsx` full rewrite with inlined layout. `_app.tsx` gets `CursorGlow` mounted globally.

**Tech Stack:** Next.js 14, React 18, TypeScript, Tailwind CSS 3, CSS keyframes (no animation library)
...
|------|--------|----------------|
| `styles/globals.css` | Modify | Add `* { cursor: none }`, `@keyframes wordReveal`, `@keyframes glowPulse`, `@keyframes fadeIn` |
| `components/CursorGlow.tsx` | Create | Fixed amber orb that lerps to mouse position, scales on interactive hover |
| `components/FilmGrain.tsx` | Create | Fixed full-screen SVG noise overlay with animated seed |
| `hooks/useCountUp.ts` | Create | `requestAnimationFrame` counter from 0 → target with easeOutQuart |
| `pages/index.tsx` | Rewrite | Full split-column hero layout wiring all animations |
| `pages/_app.tsx` | Modify | Mount `<CursorGlow />` globally |

---

### Task 1: CSS keyframes + cursor reset

**Files:**
- Modify: `styles/globals.css`

- [ ] **Step 1: Add cursor reset and keyframes to globals.css**

Append to the bottom of `styles/globals.css`:

```css
/* Custom cursor — hide default on all elements */
* {
...
/* Fade-in for non-numeric stats (∞, MIT) */
@keyframes fadeIn {
  from { opacity: 0; }
  to   { opacity: 1; }
}
...
```bash
git -C .worktrees/feat-doc-site add styles/globals.css
git -C .worktrees/feat-doc-site commit -m "feat: add wordReveal, glowPulse, fadeIn keyframes and cursor reset"
```

---

### Task 2: CursorGlow component
...
export default function CursorGlow() {
  ...
  useEffect(() => {
    ...
    const interactives = document.querySelectorAll<Element>('a, button')
    ...
    const tick = () => {
      ...
      if (elRef.current) {
        elRef.current.style.transform =
          `translate(${current.current.x - 7}px, ${current.current.y - 7}px) scale(${scale.current})`
      }
...
```bash
git -C .worktrees/feat-doc-site add components/CursorGlow.tsx
git -C .worktrees/feat-doc-site commit -m "feat: add CursorGlow component"
```

---

### Task 3: FilmGrain component

**Files:**
- Create: `components/FilmGrain.tsx`

- [ ] **Step 1: Create components/FilmGrain.tsx**
...
```bash
git -C .worktrees/feat-doc-site add components/FilmGrain.tsx
git -C .worktrees/feat-doc-site commit -m "feat: add FilmGrain component"
```
...
```bash
mkdir -p .worktrees/feat-doc-site/hooks
```
...
```bash
git -C .worktrees/feat-doc-site add hooks/useCountUp.ts
git -C .worktrees/feat-doc-site commit -m "feat: add useCountUp hook"
```

---

### Task 5: Wire CursorGlow into _app.tsx

**Files:**
- Modify: `pages/_app.tsx`

- [ ] **Step 1: Update pages/_app.tsx**
...
```bash
git -C .worktrees/feat-doc-site add pages/_app.tsx
git -C .worktrees/feat-doc-site commit -m "feat: mount CursorGlow globally in _app"
```
...
- [ ] **Step 1: Replace pages/index.tsx with the split-column magazine layout**
...
```bash
cd .worktrees/feat-doc-site && npm run dev
```

Open `http://localhost:3000`. Verify:
- Amber orb follows mouse, shrinks on hover over links
- "One ORM." / "Every" / "Database." slides up staggered on load
- "7" counts up from 0; "∞" and "MIT" fade in
- Subtle grain texture visible
- Amber glow breathes in the left column
...
```bash
git -C .worktrees/feat-doc-site add pages/index.tsx hooks/useCountUp.ts
git -C .worktrees/feat-doc-site commit -m "feat: magazine split-column hero with animations"
```
...
```bash
cd .worktrees/feat-doc-site && npm run build
```
...
```bash
git -C .worktrees/feat-doc-site add .
git -C .worktrees/feat-doc-site commit -m "chore: verify magazine hero build passes" --allow-empty
```
